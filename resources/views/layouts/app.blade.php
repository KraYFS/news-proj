...
<head>
    ...
    @if(Request::route()->getName() == 'common.main')
        <title>@yield('title')</title>
    @else
        <title>@yield('title') - {{ __('main.name') }}</title>
    @endif
    ...
    <script src="{{ asset('/script/jquery-2.2.3.js') }}"></script>
    <script src="{{ asset('/script/croppie.min.js') }}"></script>
    <script src="{{ asset('/script/jquery.maskedinput.min.js') }}"></script>
    @vite(['resources/front/scss/main.scss'])
</head>
<body>
    <div id="app">
        <header class="header">
    ...
    <nav class="header__nav">
        <ul class="nav__list">
            ...
            <li class="nav__item @if(Request::route()->getName() == 'news.main') active @endif">
                <a href="{{ route('news.main') }}" class="nav__link">
                    {{ __('main.header.news') }}
                </a>
            </li>
            <li class="nav__item @if(Request::route()->getName() == 'employees.main') active @endif">
                <a href="{{ route('employees.main') }}" class="nav__link">
                    {{ __('main.header.users') }}
                </a>
            </li>
            ...
            <li class="nav__item @if(Request::route()->getName() == 'common.contact') active @endif">
                <a href="{{ route('common.contact') }}" class="nav__link">
    ...
    <div style="display: none">
        <a href="#form_modal" class="nav__link" uk-toggle id="openModal">
          {{ __('main.main.to_command') }}
        </a>
    </div>
    ...
    <div class="lang__switcher">
        ...
        <a href="{{route('language.set', ['language' => $language->value])}}" class="lang__switch {{$language->value == app()->getLocale() ? 'active' : ''}}">{{$language->name}}</a>
        @endforeach
    </div>
    </div>
    </header>
    ...
    <div id="mobile__menu" uk-offcanvas="flip: true; overlay: true">
        <div class="uk-offcanvas-bar mobile__canvas">
            <div class="mobile__menu-wrapper">
                <div class="mobile__menu-header">
                    <button class="uk-offcanvas-close" type="button">
                        <img src="{{ asset('/img/close__menu.svg') }}" width="20" height="19" alt="">
                    </button>
                </div>
                <div class="mobile__menu">
                    <ul class="nav__list">
                        <li class="nav__item">
                            <a href="{{ route('news.main') }}" class="nav__link @if(Request::route()->getName() == 'news.main') active @endif">
                                {{ __('main.header.news') }}
                            </a>
                        </li>
                        <li class="nav__item">
                            <a href="{{ route('employees.main') }}" class="nav__link @if(Request::route()->getName() == 'employees.main') active @endif">
                                {{ __('main.header.users') }}
                            </a>
                        </li>
                        <li class="nav__item">
                            <a href="#form_modal" class="nav__link" uk-toggle>
                                {{ __('main.main.to_command') }}
                            </a>
                        </li>

                        <li class="nav__item">
                            <a href="{{ route('common.contact') }}" class="nav__link @if(Request::route()->getName() == 'common.contact') active @endif">
                                {{ __('main.header.contact') }}
                            </a>
                        </li>
    ...
    <div class="success_modal_send_form">
        <div class="close_message_success"></div>
        <div class="texts_success">
            {{ __('main.main.success_text') }}
        </div>
    </div>
</body>
